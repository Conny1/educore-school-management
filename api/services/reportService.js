...
export const getFeeCollection = async ({ schoolId, term, year }) => {
  const fees = await GradeFee.find({ schoolId, term, year }).populate(
    "gradeId",
    "name stream",
  );
...
export const getExpenseBreakdown = async ({ schoolId, startDate, endDate }) => {
  return await Expense.aggregate([
    { $match: { schoolId, is_deleted:false, expenseDate: { $gte: startDate, $lte: endDate } } },
    {
      $group: {
        _id: "$category",
        total: { $sum: "$amount" },
...
export const getRequirementFulfillment = async ({
  schoolId,
  gradeId,
  term,
  year,
}) => {
  let gradeRequirementFilter = { schoolId, isActive: true, year, term };
  if (gradeId) {
    gradeRequirementFilter["gradeId"] = gradeId;
  }

  const requirements = await GradeRequirement.find(gradeRequirementFilter);
  const totalStudents = await Student.countDocuments({ schoolId, gradeId });
  return await Promise.all(
    requirements.map(async (req) => {
      const logs = await StudentRequirementLog.find({
        schoolId,
        requirementId: req._id,
      });
      const fully = logs.filter((l) => l.qtyBrought >= req.requiredQty).length;
      const partial = logs.filter(
        (l) => l.qtyBrought > 0 && l.qtyBrought < req.requiredQty,
      ).length;
